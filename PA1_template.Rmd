...
the `mean` function to calculate the mean for every interval.
...
average.day <- mutate(average.day,timeinterval=strftime(strptime(interval, format="%H%M")))
...
purposes, the mean and median are also printed in the code below.
...
much smaller.  The median and mean move so close together that the vertical lines showing the median and mean merge into a single line on the graph.
...
weekday or a weekend day.  To create a panel plot, the `facet_grid`
layout element of `ggplot` is used.  Qualitatively, the morning peak
on weekends is smaller, and activity throughout the day is more
consistent.

TODO: add trend lines
